...
public record AE1ClipDeclaration(
  ...
  implements AE1SchemaObjectType
{
  ...
  public AUClipDeclaration toClipDeclaration()
  {
    return new AUClipDeclaration(
      ...
      this.loopRange().map(AE1ClipDeclaration::loopRangeOf)
    );
  }
  ...
  public static AE1ClipDeclaration ofDeclaration(
    final AUClipDeclaration d)
  {
    return new AE1ClipDeclaration(
      bigInteger(d.id().value()),
      d.name(),
      d.format().descriptor(),
      bigInteger(d.sampleRate()),
      bigInteger(d.sampleDepth()),
      bigInteger(d.channels()),
      d.endianness().descriptor(),
      AE1HashValue.of(d.hash()),
      bigInteger(d.size()),
      d.loopRange().map(AE1ClipLoopRange::of)
    );
  }
}
