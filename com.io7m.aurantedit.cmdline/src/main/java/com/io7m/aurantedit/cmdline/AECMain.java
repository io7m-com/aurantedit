...
import com.io7m.aurantedit.cmdline.internal.AECmdExport;
import com.io7m.aurantedit.cmdline.internal.AECmdImport;
import com.io7m.aurantedit.cmdline.internal.AECmdStatistics;
import com.io7m.aurantedit.cmdline.internal.AUVersionConverter;
import com.io7m.aurantedit.filemodel.AEVersion;
...
public final class AECMain implements Runnable
{
  private static final Logger LOG =
    LoggerFactory.getLogger(AECMain.class);
    ...
    final String[] inArgs)
  {
    ...
    final var metadata =
      new QApplicationMetadata(
        "aurantedit-cli",
        "com.io7m.aurantedit.cmdline",
        AEVersion.MAIN_VERSION,
        AEVersion.MAIN_BUILD,
        "Aurantium file editor (Command-line).",
        Optional.of(URI.create("https://www.io7m.com/software/aurantedit"))
      );

    final var converters =
      QValueConverterDirectory.core()
        .with(new AUVersionConverter());

    final var builder = QApplication.builder(metadata);
    builder.addCommand(new AECmdExport());
    builder.addCommand(new AECmdImport());
    builder.addCommand(new AECmdStatistics());
  ...
  public static int mainExitless(
    final String[] args)
  {
    final AECMain cm = new AECMain(args);
    cm.run();
    return cm.exitCode();
  }
